...
function renderDriverHistoryDetail(){
 ...
 list.querySelectorAll('.historyDetailPaste').forEach(btn=>btn.onclick=async()=>{
   ...
   const actual=[];records.forEach(r=>{const isRTS=/\b(?:return(?:ed)?\s+to\s+station|RTS|package\s+missing|pickup\s+failed)\b/i.test(r.block);if(isRTS)return;r.times.forEach(t=>{const mm=parseClock(t);if(Number.isFinite(mm)&&/\bdeliver(?:y|ies)\b/i.test(r.block))actual.push({n:r.n,m:mm,t})})});
