...
}).map(r=>{const finished=Number(r.total)>0&&Number(r.done)>=Number(r.total),last=r.lastDelivery||r._p.current?.lastDelivery||null,rescue=r._isRescue,routes=(r.routes||[r.route]).join(', ');return `<div class="item" style="${rescue?'border:2px solid #8b5cf6;background:#faf5ff':(finished?'border:2px solid #22c55e;background:#f0fdf4':(r._p.behind?'border:2px solid #dc2626':''))}"><div class="row between"><div><div class="drivername">${esc(r.name)}</div><div class="muted">${rescue?'🚑 RESCUE · '+esc(routes):esc(r.route||'')} · ${st}</div></div><span class="pill" style="${finished?'background:#dcfce7;color:#15803d':''}">${r.done||0}/${r.total||0} stops</span></div>${rescue?'<div class="row" style="margin-top:8px"><span style="display:inline-block;padding:6px 10px;border-radius:999px;background:#8b5cf6;color:white;font-size:11px;font-weight:900">🚑 RESCUE DRIVER</span><button class="btn soft coachNotRescue" data-st="'+st+'" data-key="'+esc(driverKey(r.name))+'" style="padding:6px 10px;font-size:11px">Not a rescue driver</button></div>':''}${finished?'<div style="display:inline-block;margin-top:8px;padding:6px 10px;border-radius:999px;background:#22c55e;color:white;font-size:11px;font-weight:900">✓ ROUTE COMPLETED</div>':(!r._p.routeLoaded?'<div style="display:inline-block;margin-top:8px;padding:6px 10px;border-radius:999px;background:#f59e0b;color:white;font-size:11px;font-weight:900">⚠ ROUTE NOT LOADED</div>':'')}${!finished&&r._p.routeLoaded&&!Number.isFinite(Number(r._p.current?.stop5AtMinutes))?'<div style="display:inline-block;margin-top:8px;padding:6px 10px;border-radius:999px;background:#dc2626;color:white;font-size:11px;font-weight:900">⚠ STOP 5 REAL TIME NOT FOUND</div>':''}<div class="addr" style="margin-top:10px">${finished?'COACH FINISH: '+esc(last||'Completed'):'COACH ETA: '+(r._p.eta?fmtMin(r._p.eta):'Learning...')}</div><div class="muted">Deadline ${fmtMin(mins(deadline))} · ${Number.isFinite(Number(r._p.current?.stop5AtMinutes))?'Pace since Stop 5 '+(r._p.pace?r._p.pace.toFixed(1)+'/h':'collecting data'):'Live pace '+(r._p.pace?r._p.pace.toFixed(1)+'/h':'collecting data')}${r._p.packages?' · '+r._p.packages+' packages':''}${!finished&&r._p.behind?' · 🔴 ~'+r._p.behind+' stops behind':''}</div><div class="muted" style="margin-top:4px">${r._p.model} · ${r._p.historyCount}/20 previous routes${r._p.current?.stop5AtText?' · Stop 5 '+esc(r._p.current.stop5AtText):''}</div></div>`}).join(''):'<div class="empty">No LIVE data yet. The Chrome collector will feed this station here.</div>';
