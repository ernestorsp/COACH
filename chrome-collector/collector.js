...
async function sendNow(){const data=scan(),h=new Date().getHours();if(data.station==='UNKNOWN'||h<7||h>=23||!data.drivers.length)return{...data,sent:false};return await new Promise(resolve=>chrome.runtime.sendMessage({type:'COACH_INGEST',data},r=>{if(chrome.runtime.lastError)resolve({...data,sent:false,error:chrome.runtime.lastError.message});else resolve({...data,...r})}))}
